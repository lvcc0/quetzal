...
namespace qtzl
{
    Model3D::Model3D(const std::string& name, std::shared_ptr<Mesh> mesh_sptr)
        : VisualNode3D(name), m_Mesh_sptr(mesh_sptr)
    {
        this->m_Type = Object::Type::MODEL3D;

        this->setupRender();
    }

    void Model3D::render(const std::shared_ptr<ShaderProgram>& shader_program)
    {
    ...
    void Model3D::accept(NodeVisitor& visitor)
    {
        visitor.visit(*this);
    }
}