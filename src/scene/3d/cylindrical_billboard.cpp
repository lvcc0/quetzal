...
namespace qtzl
{
	...
	void CylindricalBillboard::render(std::shared_ptr<ShaderProgram> shader_program)
	{
        ...
        this->m_HorizontalAngle = glm::acos(glm::dot(projToTarget, glm::vec3(0.0f, 0.0f, -1.0f)));
        if (this->m_HorizontalAngle < 0.01 || this->m_HorizontalAngle > 3.14)
            this->m_Up = glm::vec3(0.0f, 1.0f, 0.0f); // stability reasons

        this->m_ModelMatrix = glm::translate(this->m_ModelMatrix, this->m_Position);
        this->m_ModelMatrix = glm::rotate(this->m_ModelMatrix, this->m_HorizontalAngle, this->m_Up);
